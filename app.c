 ...
 ************************************************************************************************************************************/
uint8_t app_check_buttons(void) {
...
void app_handle_button_press() {
  ...
  if (button_current) {
    ...
    sl_simple_timer_start(&button_timer, BUTTON_TIMEOUT, app_button_timer_callback, NULL, false);
  } else {
...
void sl_bt_on_event(sl_bt_msg_t *evt) {
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    // ---------------------------------------------------------------------------------------------
    // Indicates the device has started and the radio is ready./ Do not call any stack command before receiving this boot event!
...
void throughput_peripheral_on_finish(throughput_value_t throughput, throughput_count_t count) {
  ...
  app_log_info("Throughput test finished: %d bps, %u packets", throughput, count);
  app_log_nl();
 ...
 ************************************************************************************************************************************/
void throughput_central_on_finish(throughput_value_t throughput, throughput_count_t count, throughput_count_t lost,
		                          throughput_count_t error, throughput_time_t time) {

  app_log_info("Throughput test: reception finished " APP_LOG_NEW_LINE "%d bps" APP_LOG_NEW_LINE "%u packets" APP_LOG_NEW_LINE
		       "%u lost" APP_LOG_NEW_LINE "%u error" APP_LOG_NEW_LINE "in %d sec" APP_LOG_NEW_LINE,
               throughput, count, lost, error, time);
